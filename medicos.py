...
def create_postgres_json(size):
    ...
    especialidades = ['Cardiologia', 'Hematologia', 'Neurologia', 'General', 'Oftalmologia', 'Ortopedia']
    if cur != None:
        try:
            cur.execute(''' 
            SELECT dni 
            ...
            ''')
            persona_dnis = cur.fetchall()
            print('finished QUERY execution')
        except (Exception, Error) as error:
            print("\nexecute_sql() error:", error)

    # iterate over the number of records being create
    ...
    for rec_id in range(size):
        ...
        new_record['dni'] = get_random_words_dni(persona_dnis)
        new_record['id_colegio_medico'] = ''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(8))
        new_record[ 'especialidad' ] = get_random_words(especialidades)
