import random, uuid, time, json, sys, datetime
...
def create_postgres_json(size):
    ...
    if cur != None:
        try:
            cur.execute('''
            SELECT paciente_dni, id
            FROM datos100k.historial_medico;
            ''')
            registros_hm = cur.fetchall()
            cur.execute('''
            SELECT dni
            FROM datos100k.medico;
            ''')
            medico_dni = cur.fetchall()
    ...
    words = ['Diabetes', 'Cancer', 'Osteoporosis', 'Bronquitis', 'Artritis', 'Arritmia cardiaca', 'Covid-19', 'Hipertension']
    ...
    for rec_id in range(size):
        ...
        temp = get_random_words_tuple(registros_hm)
        new_record['paciente_dni'] = temp[0]
        new_record['historial_id'] = temp[1]
        new_record['medico_dni'] = get_random_words_dni(medico_dni)
        new_record[ 'numero' ] = random.randint(1, 1000)
        new_record[ 'diagnostico' ] = get_random_words(words)
        new_record[ 'presion_sanguinea' ] = random.randint(70, 200)
        new_record[ 'fecha' ] = start_date + datetime.timedelta(days=random.randrange((end_date - start_date).days))
        new_record[ 'hora' ] = '%02d:%02d:%02d' % (hours, minutes, seconds)
        new_record[ 'talla' ] = round(random.uniform(1.2, 2.0), 2)
        new_record[ 'peso' ] = round(random.uniform(30.0, 90.0), 2)
...
if len(sys.argv) > 1:
    ...
    table_name = 'datos100k.consultas'
