...
def create_postgres_json(size):
    ...
    if cur != None:
        ...
        try:
            cur.execute('''
            SELECT dni
            FROM datos100k.paciente;
            ''')
            pacientes_dni = cur.fetchall()
    ...
    for rec_id in range(size):
        ...
        new_record['paciente_dni'] = get_random_words(pacientes_dni)
        new_record['id'] = ''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(10))

        # append the new record dict to the list
        while (True):
            encontrado = False
            for e in records:
                if e['paciente_dni'] == new_record['paciente_dni']:
                    encontrado = True
                    new_record['paciente_dni'] = get_random_words(pacientes_dni)
                if encontrado:
                    break
...
if len(sys.argv) > 1:
    ...
    table_name = 'datos100k.historial_medico'
