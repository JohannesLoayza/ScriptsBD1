...
def create_postgres_json(size):
    ...
    emails = ['hotmail', 'yahoo', 'gmail', 'outlook']
    tipos_sangre = ['A+', 'A-', 'B+', 'B-', 'A+', 'A-', 'O-', 'O+', 'AB-', 'AB+']
    ...
    for rec_id in range(size):
        ...
        new_record['dni'] = str(random.randint(10000000, 99999999))
        new_record['nombres'] = ''.join(random.choice(string.ascii_uppercase)).join(random.choice(string.ascii_lowercase) for _ in range(8))
        new_record['apellidos'] = ''.join(random.choice(string.ascii_uppercase)).join(random.choice(string.ascii_lowercase) for _ in range(8))
        new_record['fecha_nacimiento'] = start_date + datetime.timedelta(days=random.randrange((end_date - start_date).days))
        new_record['direccion'] = 'Calle ' + ''.join(random.choice(string.ascii_uppercase + string.ascii_lowercase) for _ in range(15))
        new_record['celular'] = str(random.randint(900000000, 999999999))
        new_record['correo'] = ''.join(random.choice(string.ascii_lowercase) for _ in range(5)) + '@' + get_random_words(emails) + '.com'
        new_record['fallecido'] = [True, False][random.randint(0, 1)]
        new_record['tipo_sangre'] = get_random_words(tipos_sangre)
