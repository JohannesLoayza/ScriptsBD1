...
def create_postgres_json(size):
    ...
    words = ['Farmacoterapia', 'Fisioterapia', 'Cirugia', 'Ortopedia', 'Protesis']
    if cur != None:

        try:
            cur.execute('''
            SELECT paciente_dni, historial_id, medico_dni, numero
            FROM datos100k.consultas;
            ''')
            registros_consulta = cur.fetchall()
...
table_name = 'datos100k.tratamientos'
