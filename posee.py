...
def create_postgres_json(size):
    ...
    if cur != None:
        try:
            ...
            cur.execute(''' 
            SELECT id 
            FROM datos100k.seguro;        
            ''')
            segurod_ids = cur.fetchall()
    ...
    for rec_id in range(size):
        ...
        new_record['seguro_id'] = get_random_words_dni(segurod_ids)
        new_record['persona_dni'] = get_random_words_dni(persona_dnis)
