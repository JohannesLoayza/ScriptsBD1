...
def create_postgres_json(size):
    ...
    if cur != None:
        try:
            ...
            ''')
            medico_dnis = cur.fetchall()
            cur.execute(''' 
            SELECT ruc 
            FROM datos100k.centro_salud;        
            ''')
            centro_salud_rucs = cur.fetchall()
    ...
    for rec_id in range(size):
        ...
        new_record['medico_dni'] = get_random_words_dni(medico_dnis)
        new_record['centro_salud_ruc'] = get_random_words_dni(centro_salud_rucs)
